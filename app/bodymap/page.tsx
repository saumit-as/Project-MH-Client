...
const BodyMap = () => {
  ...
  const [bodyColor, setBodyColor] = useState("#90EE90");
  ...
  function onSubmit(data: { habit: string; side: string }) {
    if (data.side === "left") {
      setNegativeHabits([...negativeHabits, data.habit]);
    } else {
      setPositiveHabits([...positiveHabits, data.habit]);
    }
  }
  function updateBodyColor() {
    const habitDifference = positiveHabits.length - negativeHabits.length;
    let backgroundColor;
    if (habitDifference > 0) {
      const greenShade = Math.min(255, 100 + 10 * habitDifference);
      backgroundColor = `rgb(0, ${greenShade}, 0)`;
    } else if (habitDifference < 0) {
      const redShade = Math.min(255, 100 + 10 * Math.abs(habitDifference));
      ...
      <div className="flex gap-5">
        <div className="flex flex-col gap-3">
          <Card>
            ...
            <CardContent>
              <ScrollArea className="h-[500px] w-[200px]">
                {negativeHabits.map((habit, index) => {
                  return (
                    <div
                      className="cursor-pointer flex items-center gap-3"
                      onClick={() => {
                        negativeHabits.splice(index, 1);
                        setNegativeHabits([...negativeHabits]);
                      }}
                      key={index}
                    >
                      {habit} <Delete size={16} />
                    </div>
                  );
                })}
              </ScrollArea>
            ...
            <CardContent>
              {positiveHabits.map((habit, index) => {
                return (
                  <p
                    onClick={() => {
                      positiveHabits.splice(index, 1);
                      setPositiveHabits([...positiveHabits]);
                    }}
                    key={index}
                  >
                    {habit}
